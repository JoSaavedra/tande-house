...
const notify = (t, p) => listeners.forEach(f => f({
  type: t,
  payload: p
}));
const persist = {
...
let state = {
  productos: persist.load(LS_KEY, window.SEED.productos),
  categorias: window.SEED.categorias,
...
window.Store = {
  ...
  createProduct(prod) {
    state.productos = [...state.productos, prod];
    persist.save(LS_KEY, state.productos);
  ...
  addToCart(item, qty = 1) {
    ...
    if (i >= 0) state.cart[i].qty += qty;else state.cart.push({
      ...item,
      qty
    });
  ...
  setQty(id, qty) {
    state.cart = state.cart.map(x => x.id === id ? {
      ...x,
      qty: Math.max(0, qty)
    } : x).filter(x => x.qty > 0);
    persist.save(LS_CART, state.cart);
