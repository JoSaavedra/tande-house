...
const regionesYComunas = {
    "Región Metropolitana de Santiago": ["Santiago", "Las Condes", "Providencia", "Ñuñoa", "La Reina", "Vitacura", "Lo Barnechea", "Maipú", "La Florida", "Peñalolén"],
    "Región de la Araucanía": ["Temuco", "Villarrica", "Pucón", "Angol", "Nueva Imperial", "Carahue", "Pitrufquén", "Lautaro", "Traiguén", "Collipulli"],
    "Región de Ñuble": ["Chillán", "San Carlos", "Quirihue", "Bulnes", "Yungay", "Coelemu", "El Carmen", "Pemuco", "Quillón", "Ninhue"]
};
...
document.getElementById('region').addEventListener('change', function() {
    ...
    // Limpiar comunas
    comunaSelect.innerHTML = '<option value="">-- Selecciona la comuna --</option>';
...
document.getElementById('loginEmail').addEventListener('blur', function() {
    if (this.value && !validateEmail(this.value)) {
        showError('loginEmailError', 'Solo se permiten correos @duoc.cl, @profesor.duoc.cl y @gmail.com');
    } else if (this.value) {
...
document.getElementById('registerEmail').addEventListener('blur', function() {
    if (this.value && !validateEmail(this.value)) {
        showError('registerEmailError', 'Solo se permiten correos @duoc.cl, @profesor.duoc.cl y @gmail.com');
    } else if (this.value) {
...
document.getElementById('loginPassword').addEventListener('blur', function() {
    if (this.value && !validatePassword(this.value)) {
        showError('loginPasswordError', 'La contraseña debe tener entre 4 y 10 caracteres');
    } else if (this.value) {
...
document.getElementById('registerForm').addEventListener('submit', function(e) {
    ...
    if (users.find(user => user.email === userData.email)) {
        showAlert('registerAlert', 'Ya existe un usuario con este correo electrónico', 'error');
        hasErrors = true;
    }
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    if (user) {
        ...
        setTimeout(() => {
            alert(`Login exitoso! Bienvenido ${user.fullName}\nRegión: ${user.region}\nComuna: ${user.comuna}`);
        }, 1500);
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentUser) {
        ...
        showAlert('loginAlert', `Ya tienes una sesión activa como ${user.fullName}`, 'success');
    }
});